...
class UserProfile(models.Model):
    ...
    profile_image = models.ImageField(upload_to='images/profile-image/%Y/%m/%d/', null=True, blank=True)
    ...
    def get_profile_image_url(self):
        ...
        dummy_url = 'https://i.pravatar.cc/300?u={username}'.format(username=self.user.username)
...
def user_profile_create_reviever(sender, instance, created, *args, **kwargs):
    if created:
        try:
            user, user_created = UserProfile.objects.get_or_create(user=instance)
        except:
